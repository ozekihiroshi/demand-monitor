...
class SeriesController extends Controller
{
    ...
    public function series(Request $req, string $code)
    {
        ...
        if (! in_array($view, ['overlay', 'timeline'], true)) {
            $view = 'overlay';
        }
        ...
        if ($bucket === '1m' && $req->filled('since')) {
            ...
            if ($sinceMin->lt($spanStart)) {
                $sinceMin = $spanStart->copy();
            }

            // demand テーブル読み出し（存在しない環境では空配列が返る）
            $raw  = DemandTable::read($key, $sinceMin->timestamp, $cursor->timestamp);
            // resample 系は ['t','kw'] を想定しているため整形
            $rows = array_map(fn($r) => ['t' => (int)$r['date'], 'kw' => (float)$r['data']], $raw);
        ...
        $raw  = DemandTable::read($key, $start->timestamp - $padBefore, $end->timestamp + $padAfter);
        $rows = array_map(fn($r) => ['t' => (int)$r['date'], 'kw' => (float)$r['data']], $raw);
    ...
    private function resample1m_range(Carbon $sinceMin, Carbon $endMin, array $points, string $tz): array
    {
        ...
        $b = [];
        foreach ($points as $p) {
            if ($p['t'] < $sinceMin->timestamp || $p['t'] > $endMin->timestamp) {
                continue;
            }
    ...
    private function resample1m_span(Carbon $start, Carbon $end, array $points, string $tz): array
    {
        ...
        $b = [];
        foreach ($points as $p) {
            if ($p['t'] < $start->timestamp || $p['t'] > $end->timestamp) {
                continue;
            }
    ...
    private function aggregate30m_trapezoid(Carbon $day, array $points, string $tz): array
    {
        ...
        $ends = [];for ($e = $d0 + 1800; $e <= $d1; $e += 1800) {
            $ends[] = $e;
        }
        ...
        $interp = function (int $t) use ($points, $n): ?float {
            ...
            if ($t < $points[0]['t'] || $t > $points[$n - 1]['t']) {
                return null;
            }
        ...
        foreach ($ends as $e) {
            ...
            $vS = $interp($s);
            $vE = $interp($e);
            if ($vS === null || $vE === null) {$out[] = [$iso($e), null];
                continue;}
            $inside = array_values(array_filter($points, fn($p) => $p['t'] > $s && $p['t'] < $e));
            $knots  = [['t' => $s, 'kw' => $vS]];
            foreach ($inside as $p) {
                $knots[] = ['t' => $p['t'], 'kw' => (float) $p['kw']];
            }
